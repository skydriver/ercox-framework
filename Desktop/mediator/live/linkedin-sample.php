...
if( email_exists( $user->emailAddress ))
{
    codeart_linkedin_print_error('Email address exists.');
}
else
{
    global $member;

    $new_user = $member->register_with_linkedin($user);

    if($new_user)
    {
        wp_redirect( get_field('thank_you_page', 'option'), 301 );
        exit;
    }
}
